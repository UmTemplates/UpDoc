...
namespace TailoredTravel.Web.DataSources
{
    public sealed class FeatureBlockDataSource : IContentmentDataSource
    {
        ...
        public string Group => "Tailored Travel";
        ...
        private static readonly Dictionary<string, string> IconByAlias = new(StringComparer.OrdinalIgnoreCase)
        {
            ["featureRichTextEditor"] = "tt-square-pen",
            ["featureRichTextEditorAccommodation"] = "tt-bed-double",
            ["featureRichTextEditorFeatures"] = "tt-concierge-bell",
            ["featureRichTextEditorSights"] = "tt-milestone",
            ["featureRichTextEditorSightsMore"] = "tt-signpost-big",
            ["featureRichTextEditorExtras"] = "tt-package-plus",
        };
            ...
            foreach (var block in grid)
            {
                ...
                if (alias.StartsWith("feature", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    items.Add(new DataListItem
                    {
                        ...
                        Group = groupName,
                        Icon = IconByAlias.TryGetValue(alias, out var icon) ? icon : null,
                    });
                }
                    ...
                    foreach (var nested in area)
                    {
                        ...
                        if (nestedAlias.StartsWith("feature", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            items.Add(new DataListItem
                            {
                                ...
                                Group = groupName,
                                Icon = IconByAlias.TryGetValue(nestedAlias, out var nestedIcon) ? nestedIcon : null,
                            });
                        }
                    }
                }
